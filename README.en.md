...
| Protocol | URL | Usage |
|-----------|-----|-------|
| Local HTTP | `http://192.168.10.155:11434` | Internal network only |
...
4ride.online DNS A 130.180.208.168 (bbox public IP)

 Bbox (router) 
 Port 443 → 8443 
 Port 80 → 11434 

 192.168.10.155 (IA-SERVER, local network)
...
The bbox then forwards traffic to this server (`192.168.10.155`) via port forwarding rules.
